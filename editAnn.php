...
if(!isset($_GET["id"]) || !is_numeric($_GET["id"]) || $_GET["id"] <= 0){
    ...
    try {
        ...
        echo '<input type="text" name="titleEdit" value="'.htmlspecialchars($announcement->title).'">';
        echo '<p>'.htmlspecialchars($announcement->location)." ".$announcement->date.'</p>';
        echo '<h2><i class="fas fa-shopping-cart"></i> <input type="number" name="valueEdit" value='.$announcement->value.'> zł</h2>';
        echo '</div>';
...
<section>
    <div id="description">
        <h2>Opis</h2>
        <p> <textarea name="descriptionEdit" rows="15" cols="230"> <?php echo htmlspecialchars("$announcement->description") ?></textarea><button type="submit" name="edit">Zakończ edytowanie</button></p>
    </div>
    </form>
</section> 
